...
#initial concentration 
  ...
  HIFan_aOH <- 0
  HIFan_aOHpOH <- 0
  PHD <- 0 #set normal concentration: 100
  PHDn <- 0
  HIFb <- 170
  HRE <- 50
  mRNA <- 0
...
for (i in c(2:28800-1)){
 ...
 #simulate the process
  HIFa <- HIFa + (V1-V2-V9+V10-V3-V5+V6)*dt
  HIFa_pOH <- HIFa_pOH + (V3-V4)*dt
  HIFa_aOH <- HIFa_aOH + (V5-V6-V7-V13+V14)*dt
  HIFa_aOHpOH <- HIFa_aOHpOH + (V7-V8)*dt
...
#plot 
  ...
  plot(time,s_HIFa,col ="red", type = "l",xlab = "time(h)",ylab = "HIF¦Á concentration(nM)")
  lines(time,s_HIFa,col ="black")
  legend("topleft",legend = c("HIF¦Á","HIF-HRE"),col = c("blue","red"),lty = 1, cex = 1)
  
  #plot of "protein & PHD"
  plot(time,s_PHD,col ="red", type = "l",ylim = c(0,100), xlab = "time(h)",ylab = "concentration(nM)")
